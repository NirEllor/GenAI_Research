...
- `generate_teacher_datasets.py` — data-generation step for a teacher/student distillation study: for each already-trained per-latent-dim Latent-CFM checkpoint (the "teacher", from `train_cifar10_ddp_vae_cond_ic.py`), Euler-samples independent synthetic image datasets at several sizes (`--dataset_sizes`; each sample stores the starting noise `x0` (t=0), the final generated image `x1` (t=1), and the conditioning latent used) and one trajectory dataset (a strided subset of intermediate Euler states, for future students trained on "middle-time" targets instead of only the final image). Reuses the exact conditioning/sampling logic from `compute_fid.py`'s `gen_1_img` (real CIFAR batch → AE latent → reparameterize through the UNet's own `latent_encodings` → Euler-integrate noise to image); writes self-describing `.npy` files via `np.lib.format.open_memmap` plus a `manifest.json`, with atomic `.tmp` + rename and a `.done` marker per output so partial runs can't be mistaken for complete ones on `--overwrite`-less reruns. Expects the checkpoint layout produced by `slurm/run_train_fm.sh` (`{--input_dir}/latent_{dim}/{--model}/...`).
- `train_students.py` — distills each per-latent-dim teacher into a one-step "student": for every (latent dim, dataset size) pair (6×4=24 by default), loads that dim/size's `(x0, x1)` pairs from `generate_teacher_datasets.py`'s output and trains a smaller `UNetModelWrapper` (no latent conditioning; `--student_num_channels`/`--student_num_res_blocks` default to half the teacher's) to predict the single global velocity `v = x1 - x0` evaluated at `x0` with `t` fixed at 0 — so a trained student generates a sample in one forward pass (`x1_pred = x0 + student(x0)`), no ODE integration needed. AdamW + cosine LR + EMA + early stopping, mirroring the hyperparameters of the source distillation script this was adapted from. Checkpoints go to `{...}/students/student_{dim}_{n_samples}.pt`; skips any that already exist unless `--overwrite`.
