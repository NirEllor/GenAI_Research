...
- `generate_teacher_datasets.py` — data-generation step for a teacher/student distillation study: for each already-trained per-latent-dim Latent-CFM checkpoint (the "teacher", from `train_cifar10_ddp_vae_cond_ic.py`), Euler-samples independent synthetic image datasets at several sizes (`--dataset_sizes`, plus the per-sample conditioning latent used) and one trajectory dataset (a strided subset of intermediate Euler states, for future students trained on "middle-time" targets instead of only the final image). Reuses the exact conditioning/sampling logic from `compute_fid.py`'s `gen_1_img` (real CIFAR batch → AE latent → reparameterize through the UNet's own `latent_encodings` → Euler-integrate noise to image); writes self-describing `.npy` files via `np.lib.format.open_memmap` plus a `manifest.json`, with atomic `.tmp` + rename and a `.done` marker per output so partial runs can't be mistaken for complete ones on `--overwrite`-less reruns. Expects the checkpoint layout produced by `slurm/run_train_fm.sh` (`{--input_dir}/latent_{dim}/{--model}/...`).
