...
def generate_sample_trajectories(parallel, savedir, step, net_="normal",train_sample=None,vae=None):
    ...
    with torch.no_grad():
        latent = torch.randn_like(x1) #vae_(x1).view(x1.size(0),-1)
        traj = vae_.decode(latent)
        traj = traj.repeat(10,1,1,1)
        
    
    plot_darcy(traj[:,0].cpu().numpy(), savedir + f"{net_}_generated_KDE_FM_images_step_{step}_base_vae_medium_P.png")
    plot_darcy(traj[:,1].cpu().numpy(), savedir + f"{net_}_generated_KDE_FM_images_step_{step}_base_vae_medium_K.png")
    vae_.train()
...
def train(rank, total_num_gpus, argv):
    ...
    optim = torch.optim.Adam(list(vae.parameters()), lr=FLAGS.lr)
        ...
        for epoch in epoch_pbar:
            ...
            with trange(steps_per_epoch, dynamic_ncols=True) as step_pbar:
                for step in step_pbar:
                    ...
                    out, encoder_output = vae(x1)
                    mu, logvar = torch.chunk(encoder_output, 2, dim=1)
                    mse_loss = torch.mean((x1 - out) ** 2)
                    ...
                    if (FLAGS.save_step > 0 and global_step % FLAGS.save_step == 0) or global_step == 1:
                        generate_sample_trajectories(
                             FLAGS.parallel, savedir, global_step, net_="normal", train_sample=mu[:10], vae=vae
                        )
                        generate_sample_trajectories(
                             FLAGS.parallel, savedir, global_step, net_="ema", train_sample=mu[:10], vae=vae
                        )
                        torch.save(
                            {
                                "vae": vae.state_dict(),
                            ...
                            savedir + f"{FLAGS.model}_cifar10_kde_weights_step_{global_step}_base_vae_medium.pt",
                        )
