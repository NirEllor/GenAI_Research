...
- Constructor gains `num_latents` (dimensionality of the incoming VAE latent) and `latent_dim` (width of the internal reparameterization bottleneck).
...
- **Used by**: `code/cifar10/train_cifar10_ddp_vae_cond_ic.py` (set `num_latents=latent_dim` to the experiment's AE dim, so the internal bottleneck matches the full AE latent) and `code/darcy_flow/train_cifar10_ddp_vae_cond_ic_medium.py` (`num_latents=8*8*8=512` fixed, `latent_dim` CLI-configurable via `--latent_dim`, e.g. set to `2` for the 2D-latent experiments).
...
- **one** synthetic `(x0, x1, latent)` triple pool per dim, of size `--dataset_size` (default 200,000; `noise_fp16.npy` = starting noise x0 at t=0, `images_uint8.npy` = the teacher's final sample x1 at t=1, `latents_fp32.npy` = the **raw AE latent** — `ae.encode(img)[0]`, shape `(dataset_size, dim)` — that conditioned that sample) — this repo's `x0`=noise/`x1`=data convention, per `train_cifar10_ddp_vae_cond_ic.py`, not the reversed convention some earlier drafts of this pipeline used. `encode_conditioning_latent` returns `(ae_latent, teacher_y)`: `ae_latent` is what's persisted (and what the student is conditioned on), while `teacher_y` is the teacher's own reparameterized projection of it (matches the AE latent dimension for that dim) used only to drive that sample's Euler integration — the teacher's internal `latent_encodings`/`latent_mlp` layers are sized to match `num_latents=dim`, so the conditioning bottleneck is never artificially compressed. **This single pool is generated once per dim, not once per `--dataset_sizes` entry** — `train_students.py` trains each dataset-size experiment on a `[0:n]` prefix of it (see Step 3), so a 100k-sample experiment is a strict superset of the corresponding 50k one rather than an independently resampled dataset.
